namespace ArticleDomain
{
    ...
    internal static class Assert
    {
        ...
        {
            if (obj == null) throw new ArticleDomainArgumentException(string.Format("{0}不能为空", name));
        }

        public static void IsNotNullOrEmpty(string name, string input)
        {
            if (string.IsNullOrEmpty(input)) throw new ArticleDomainArgumentException(string.Format("{0}不能为空", name));
        }
        ...
        {
            if (id1 != id2) throw new ArticleDomainArgumentException(string.Format(errorMessageFormat, id1, id2));
        }
    }
}